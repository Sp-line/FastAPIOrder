...
@fs_router.subscriber(
    ...
    pull_sub=True,
    durable=HallDurables.SESSION_SVC_HALLS_CREATED_SYNC_DB,
...
async def halls_created_on_session_microservice_sync_db(
    ...
    async with inbox_unit_of_work.transactional(
            msg_id=msg_id,
            handler=HallDurables.SESSION_SVC_HALLS_CREATED_SYNC_DB,
    ) as should_proceed:
        if should_proceed:
...
@fs_router.subscriber(
    ...
    stream=showtimes_stream,
    pull_sub=True,
    durable=HallDurables.SESSION_SVC_HALLS_BULK_CREATED_SYNC_DB,
    ack_policy=settings.faststream.consumer.faststream_ack_policy,
    config=base_consumer_config
)
async def halls_bulk_created_on_session_microservice_sync_db(
    ...
    async with inbox_unit_of_work.transactional(
            msg_id=msg_id,
            handler=HallDurables.SESSION_SVC_HALLS_BULK_CREATED_SYNC_DB,
    ) as should_proceed:
        if should_proceed:
...
@fs_router.subscriber(
    ...
    stream=showtimes_stream,
    pull_sub=True,
    durable=HallDurables.SESSION_SVC_HALLS_UPDATED_SYNC_DB,
    ack_policy=settings.faststream.consumer.faststream_ack_policy,
    config=base_consumer_config
)
async def halls_updated_on_session_microservice_sync_db(
    ...
    async with inbox_unit_of_work.transactional(
            msg_id=msg_id,
            handler=HallDurables.SESSION_SVC_HALLS_UPDATED_SYNC_DB,
    ) as should_proceed:
        if should_proceed:
...
@fs_router.subscriber(
    ...
    stream=showtimes_stream,
    pull_sub=True,
    durable=HallDurables.SESSION_SVC_HALLS_BULK_UPDATED_SYNC_DB,
    ack_policy=settings.faststream.consumer.faststream_ack_policy,
    config=base_consumer_config
)
async def halls_bulk_updated_on_session_microservice_sync_db(
    ...
    async with inbox_unit_of_work.transactional(
            msg_id=msg_id,
            handler=HallDurables.SESSION_SVC_HALLS_BULK_UPDATED_SYNC_DB,
    ) as should_proceed:
        if should_proceed:
